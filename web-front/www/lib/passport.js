...
app.get('/flash-display',function(req,res){
  ...
  res.send(fmsg);
    ...
    passport.deserializeUser(function(id, done) {
      ...
      var sql = 'SELECT * FROM customer CROSS JOIN (SELECT * FROM photographer) WHERE id=?';
...
passport.use(new LocalStrategy(
  ...
  function(username,password,done){
    var sql ='SELECT * FROM customer CROSS JOIN (SELECT * FROM photographer)CNT WHERE id=?';
    db2.query(sql, [username], function(err,results){
      
      if(err)
        return done(err);
      if(!results[0])
        return done('please check your name.');
